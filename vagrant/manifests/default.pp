...
# Create a symlink from ~/mapAppDj to /vagrant as a convenience for the developer
file {'/home/vagrant/mapAppDj':
    ensure => link,
    target => '/vagrant',
}
...
exec {'create-virtualenv':
    ...
    user => 'vagrant',
    group => 'vagrant',
    require => [ Package['virtualenvwrapper'], File['/home/vagrant/mapAppDj'], File['/etc/profile.d/oracle.sh'],
                 Exec['known_hosts'], ],
    ...
    creates => '/home/vagrant/.virtualenvs/mapAppDj',
}

# set the DJANGO_SETTINGS_MODULE environment variable
file_line {'add DJANGO_SETTINGS_MODULE env to postactivate':
    ensure => present,
    line => 'export DJANGO_SETTINGS_MODULE=mapAppDj.settings.local',
    path => '/home/vagrant/.virtualenvs/mapAppDj/bin/postactivate',
...
file_line {'add DJANGO_SETTINGS_MODULE env to postdeactivate':
    ...
    path => '/home/vagrant/.virtualenvs/mapAppDj/bin/postdeactivate',
    require => Exec['create-virtualenv'],
}
...
exec {'init-db-and-migrate':
    ...
    unless => 'psql -lqt | cut -d \| -f 1 | grep -wq mapAppDj',
    logoutput => true,
}
...
echo "Activating python virtual environment \"mapAppDj\""
workon mapAppDj
    ',
    require => Exec['create-virtualenv'],
}
